"""
EPL Trajectory Clusters — Visualization Generator
Run: python3 generate_trajectories.py  →  trajectories.html
...
seasons = d["seasons"]   # 35 seasons, 1991/92 → 2025/26
teams   = d["teams"]     # 52 clubs, pos[] array — None when not in EPL
...
for i, name in enumerate(team_names):
    c = int(labels[i])
    if c not in raw_clusters:
        raw_clusters[c] = []
    feats = extract_features(teams[name]["pos"])
    raw_clusters[c].append((name, feats))

# Sort clusters: most seasons first → permanent fixtures lands first
def cluster_sort_key(c_id):
    ...
    # permanent: high seasons + high current + early first
    # yo-yo: mid seasons + mid current
    # modern: high first (late arrival) + some current
    # departed: low seasons + 0 current
...
<p class="subtitle">k-means clustering · 52 clubs · 35 seasons (1991/92–2025/26) · 6 features</p>
