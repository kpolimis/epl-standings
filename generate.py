"""Build index.html — animated EPL league positions bump chart.

Reads RAW_DATA (hardcoded standings) and writes a self-contained
D3.js single-page chart. Also writes data/standings.json for downstream
scripts (generate_trajectories.py, cluster_trajectories.py).
...
def get_initials(name: str) -> str:
    """Derive a 1–3 letter abbreviation for a club name.

    Args:
        name: Full club name.

    Returns:
        Uppercase initials from INITIALS_OVERRIDE, or the first letter of
        each significant word (excluding common football suffixes).
    """
    if name in INITIALS_OVERRIDE:
        return INITIALS_OVERRIDE[name]
    skip = {"&", "FC", "AFC", "City", "United", "Town", "Athletic", "County", "Wanderers", "Rovers"}
    words = [w for w in name.split() if w not in skip] or name.split()
    return "".join(w[0] for w in words[:3]).upper()


def wl(filename: str) -> str:
    """Build a Wikipedia Special:FilePath URL for a Wikimedia Commons SVG.

    Args:
        filename: The SVG filename on Wikimedia Commons.

    Returns:
        Full Wikipedia FilePath URL string.
    """
...
def main() -> None:
    ...
    *,*::before,*::after{{box-sizing:border-box;margin:0;padding:0}}
    ...
    logger.info("data/standings.json written")
